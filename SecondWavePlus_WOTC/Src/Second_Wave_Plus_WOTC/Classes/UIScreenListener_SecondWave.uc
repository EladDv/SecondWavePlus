...
simulated public function OnContinueButton(UIButton Button)
{
	...
	if( UIScreen_SecondWaveOptions(`SCREENSTACK.GetCurrentScreen()) == none)
		return;

	UIScreen_SecondWaveOptions(`SCREENSTACK.GetFirstInstanceOf(class'UIScreen_SecondWaveOptions')).ContinueButton(Button);
	if(SecondWave_NotCreatedEqually_GameState(`XCOMHISTORY.GetSingleGameStateObjectForClass(class'SecondWave_NotCreatedEqually_GameState')) == none)
	{
	...
	`log("Updated SWP...",,'Second Wave Plus');
}
...
event OnInit(UIScreen Screen)
{
	...
	local SecondWave_CommandersChoice_GameState ComChoA;
	...
	`XEVENTMGR.RegisterForEvent(Myself,'LaunchMissionSelected',MissionLaunched,ELD_Immediate);	
	//`XEVENTMGR.RegisterForEvent(Myself,'LaunchMissionSelected',MissionLaunched,ELD_OnStateSubmitted);	
	//`XEVENTMGR.RegisterForEvent(Myself,'OnUnitBeginPlay',OnUnitBeginPlay,ELD_OnStateSubmitted);	
	if(Screen.IsA('UIShellDifficulty'))
	{
		UIShellDifficulty(Screen).m_TutorialMechaItem.SetDisabled(true,"Second Wave Plus Dosnt allow for tutorial");
		UIShellDifficulty(Screen).m_bControlledStart = false;
	...
	{
		ComChoA = SecondWave_CommandersChoice_GameState(`XCOMHISTORY.GetSingleGameStateObjectForClass(class'SecondWave_CommandersChoice_GameState'));
		//if( ComChoA.bIs_CommandersChoice_Activated)
	...
	if(Screen.IsA('UIFacilityGrid'))
	{
		...
		HidPA.InitListeners();
		NCEA.InitListeners();	
		//class'SecondWave_RedFog_GameState'.static.GCValidationChecks();
	...
	if(Screen.IsA('UIRecruitSoldiers'))
	{
		UIRecruitSoldiers(Screen).List.OnSelectionChanged=OnRecruitChangedOverride;
	}

	if((`TACTICALRULES != none || `TACTICALRULES.TacticalGameIsInPlay()) && !`REPLAY.bInReplay)	
	{
		RFA=SecondWave_RedFog_GameState(`XCOMHISTORY.GetSingleGameStateObjectForClass(class'SecondWave_RedFog_GameState'));
		RFA.InitListeners();
	}
	if(UISkyrangerArrives(Screen) != none)
	{
		class'SecondWave_RedFog_GameState'.static.GCValidationChecks();
	}
}

function EventListenerReturn MissionLaunched(Object EventData, Object EventSource, XComGameState GameState, Name EventID, Object CallbackData)
{
	`log("WE ARE HERE!!!!!");
...
//	local SecondWave_HiddenPotential_GameState HidPA;
	...
	if(Screen.IsA('UIArmory_Promotion'))
	{
		`log("Activating Hidden Potential");
		`XEVENTMGR.TriggerEvent('HiddenPotential_ApplyUpdate',
								XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(UIArmory_Promotion(Screen).UnitReference.ObjectID)),
								XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(UIArmory_Promotion(Screen).UnitReference.ObjectID)) );
	}
...
/*function AddCommandersChoiceSpinner(UIArmory_Promotion PromotionScreen)
{
	...
	CC_ClassDisplayNames.Length=0;
	CC_ClassNames.Length=0;
	if( SecondWave_CommandersChoice_GameState(`XCOMHISTORY.GetSingleGameStateObjectForClass(class'SecondWave_CommandersChoice_GameState')).
		ExcludedClasses.find(string(PromotionScreenUnit.GetSoldierClassTemplate().DataName)) != -1)
		return;
