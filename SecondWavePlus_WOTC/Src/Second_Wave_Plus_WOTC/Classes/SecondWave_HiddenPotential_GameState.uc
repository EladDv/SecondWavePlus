...
function EventListenerReturn HiddenPotentialStart(Object EventData, Object EventSource, XComGameState NewGameState, Name InEventID, Object CallbackData)
{
...
function array<HiddenPotentialLevelChanges> CreateHiddenPotentialRanks(XComGameState_Unit Unit)
{
	...
	for(i=0;i<Unit.GetSoldierClassTemplate().GetMaxConfiguredRank();i++)
	{
		...
		{
			if(SingleSoldierClassProgression.StatType != eStat_CombatSims && SingleSoldierClassProgression.StatAmount > 0 )
			{
				...
				`log("Stat Progression:" @SingleSoldierClassProgression.StatType @SingleSoldierClassProgression.StatAmount);
				`log("Random Limit:"@Templog*-1 @"-"@Templog,,'Second Wave Plus-Hidden Potential');
				TempStatChange.Change=RandActor.GetRandomStat(int(SingleSoldierClassProgression.StatAmount*HiddenPotentialRandomPercentage/100.00),,true);
...
function EventListenerReturn HiddenPotentialApplyUpdate(Object EventData, Object EventSource, XComGameState NewGameState, Name InEventID, Object CallbackData)
{	
	if(bIs_HiddenPotential_Activated)
	{
