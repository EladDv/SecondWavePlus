//---------------------------------------------------------------------------------------
//  FILE:   XComDownloadableContentInfo_Second_Wave_Plus_WOTC.uc                                    
//           
...
class X2DownloadableContentInfo_Second_Wave_Plus_WOTC extends X2DownloadableContentInfo;
...
{
	local AbilitySetupData Data;
	local SecondWave_RedFog_GameState				Main_RedFog_GameState;

	if(`REPLAY.bInReplay) return;

	`log("ADDING ABILITY TO UNIT:" @UnitState.GetFullName(),,'Second Wave Plus');
	...
	Data.TemplateName = 'RedFog_SecondWavePlus';
	Data.Template = class'X2AbilityTemplateManager'.static.GetAbilityTemplateManager().FindAbilityTemplate('RedFog_SecondWavePlus');

	if( (UnitState.IsSoldier() && !Main_RedFog_GameState.b_IsRedFogActive_XCom) ||
			(UnitState.IsAlien()&& !Main_RedFog_GameState.b_IsRedFogActive_Aliens) ||
			(UnitState.IsAdvent() && !Main_RedFog_GameState.b_IsRedFogActive_Advent) ||
			(UnitState.IsRobotic() && !Main_RedFog_GameState.b_IsRedFogActive_Robotics) ||
			 !Main_RedFog_GameState.b_IsRedFogActive)
			return;
